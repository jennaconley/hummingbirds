...
# # changed. It's resource-intensive so only use while debugging
...
def begin_search():
    ...
    if cache.get(current_month):
        ...
        checklist_location_list = (
            db.session.query(Checklist, Location)
            .join(Checklist.location)
            .filter(extract("month", Checklist.datetime_object) == current_month)
            .all()
        )

        post_query_time = time.time()
        print(
            f"We just finished the main query. It took {post_query_time - beginning} seconds."
        )
        ...
        for location_object in location_set:
            lat_long_tuples_set.add(
                (
                    round(location_object.latitude, 1),
                    round(location_object.longitude, 1),
                )
            )

        post_sets_time = time.time()
        print(
            f"We just finished the location condensing loops.  It took {post_sets_time - post_query_time}"
        )
    ...
    return render_template(
        "homepage.html",
        humlist=birds,
        lat_long_tuples_set=lat_long_tuples_set,
...
def show_sightings(ebird_id):
    ...
    print(
        f"We just finished the BirdType query. It took {post_BirdType_time - beginning} seconds"
    )
    ...
    if cache.get(ebird_id):
        ...
        sighting_checklist_location_list = (
            db.session.query(BirdSighting, Checklist, Location)
            .filter(BirdSighting.ebird_id == ebird_id)
            .join(BirdSighting.checklist)
            .join(Checklist.location)
            .all()
        )
        post_sightings_time = time.time()
        print(
            f"We just finished the BirdSighting, Checklist, Location query. It took {post_sightings_time - beginning} seconds"
        )
        ...
        for (
            ...
            checklist_object,
            location_object,
        ) in sighting_checklist_location_list:
        ...
        print(
            f"We just finished the loc_birdcount_dict loop.  This last part took {post_birdcount_time - post_sightings_time} seconds"
        )
        ...
        for loc_code in loc_birdcount_dict.keys():
            ...
            current_dict = {
                ...
                "circle_size": math.sqrt(loc_birdcount_dict[loc_code]),
            }
        ...
        print(
            f"We just finished the current_dict loop.  It took {post_current_dict_time - post_birdcount_time}"
        )
